        ...
        if (LogitechGSDK.LogiUpdate() && LogitechGSDK.LogiIsConnected(0))
        {
            ...
            if (rec.lRz * rec.lY < 0 )
            {
                ...
                if(rec.lY < 0)
                {
                 ...
                 car.transform.position += new Vector3(Mathf.Abs(rec.lY) / 32767 * 4 * Time.deltaTime, 0f, 0);


                } 


            } else if(rec.lRz * rec.lY > 0)
            {
            ...
            if (Mathf.Abs(rec.lX) > 0)
            {


                car.transform.position += new Vector3(0,0,Mathf.Abs(rec.lY) / 32767 * 4 * Time.deltaTime * rec.lX / 32767 * Time.deltaTime * 4);

               }


            rotationSpeed = rec.lX;
